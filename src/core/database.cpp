...
std::string po_status_to_str(PhoneOrderStatus s) {
    switch (s) {
        case PhoneOrderStatus::HOLDING:
            return "HOLDING";
        case PhoneOrderStatus::COMPLETED:
            return "COMPLETED";
        case PhoneOrderStatus::CANCELLED:
            return "CANCELLED";
    }
    return "HOLDING";
}
...
void Database::migrate() {
    ...
    std::cout << "[database] schema v" << current << " → v" << kLatestVersion
              << " — running migrations\n";
...
std::string Database::load_setting(const std::string& key, const std::string& default_val) const {
    sqlite3_stmt* stmt = nullptr;
...
std::vector<MenuItem> Database::load_menu() {
    ...
    sqlite3_stmt* stmt = nullptr;
    sqlite3_prepare_v2(db_,
                       "SELECT id,name,price_cents,category,send_to_kitchen FROM menu_items ORDER "
                       "BY category,name",
                       -1, &stmt, nullptr);
    ...
    for (auto& mi : items) {
        sqlite3_prepare_v2(
            db_, "SELECT id,name,min_select,max_select FROM modifier_groups WHERE menu_item_id=?",
            -1, &stmt, nullptr);
        ...
        for (auto& g : mi.modifier_groups) {
            sqlite3_prepare_v2(
                db_, "SELECT id,name,price_cents,is_default FROM modifiers WHERE group_id=?", -1,
                &stmt, nullptr);
...
void Database::save_ticket(const Ticket& t) {
    ...
    stmt = nullptr;
    sqlite3_prepare_v2(db_, "DELETE FROM ticket_items WHERE ticket_id=?", -1, &stmt, nullptr);
    sqlite3_bind_text(stmt, 1, t.id.c_str(), -1, SQLITE_TRANSIENT);
    sqlite3_step(stmt);
    sqlite3_finalize(stmt);
    ...
    // Replace payments.
    stmt = nullptr;
    sqlite3_prepare_v2(db_, "DELETE FROM payments WHERE ticket_id=?", -1, &stmt, nullptr);
    sqlite3_bind_text(stmt, 1, t.id.c_str(), -1, SQLITE_TRANSIENT);
    sqlite3_step(stmt);
    sqlite3_finalize(stmt);
    ...
    sqlite3_prepare_v2(
        db_, "INSERT INTO payments(ticket_id,payment_type,amount_cents,seq) VALUES(?,?,?,?)", -1,
        &ins_pay, nullptr);
...
std::vector<Ticket> Database::load_all_tickets() {
    ...
    for (auto& t : tickets) {
        ...
        stmt = nullptr;
        sqlite3_prepare_v2(
            db_, "SELECT payment_type,amount_cents FROM payments WHERE ticket_id=? ORDER BY seq",
            -1, &stmt, nullptr);
        sqlite3_bind_text(stmt, 1, t.id.c_str(), -1, SQLITE_TRANSIENT);
        while (sqlite3_step(stmt) == SQLITE_ROW) {
            Payment p;
...
std::vector<PhoneOrder> Database::load_all_phone_orders() {
    ...
    sqlite3_stmt* stmt = nullptr;
    sqlite3_prepare_v2(
        db_, "SELECT id,ticket_id,customer_name,comment,status,created_at_ms FROM phone_orders", -1,
        &stmt, nullptr);
    while (sqlite3_step(stmt) == SQLITE_ROW) {
        PhoneOrder po;
...
void Database::save_report(const DailyReport& rpt) {
    ...
    sqlite3_prepare_v2(
        ...
        "comped_total_cents=excluded.comped_total_cents, "
        "refunded_total_cents=excluded.refunded_total_cents, "
        "net_revenue_cents=excluded.net_revenue_cents, "
        "cc_fee_total_cents=excluded.cc_fee_total_cents, "
    ...
    // Replace item sales.
    stmt = nullptr;
    sqlite3_prepare_v2(db_, "DELETE FROM report_item_sales WHERE report_date=?", -1, &stmt,
                       nullptr);
    sqlite3_bind_text(stmt, 1, rpt.date.c_str(), -1, SQLITE_TRANSIENT);
    sqlite3_step(stmt);
    sqlite3_finalize(stmt);
...
std::vector<DailyReport> Database::load_all_reports() {
    ...
    sqlite3_prepare_v2(db_,
                       ...
                       "refunded_total_cents,net_revenue_cents,cc_fee_total_cents,"
                       "total_collected_cents,subtotal_cents "
                       "FROM archived_reports ORDER BY date DESC",
                       -1, &stmt, nullptr);
